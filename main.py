...
def yellow_handle_movement(keys_pressed, yellow):
    if keys_pressed[pygame.K_a] and yellow.x - VEL > 0:
        yellow.x -= VEL
    if keys_pressed[pygame.K_d] and yellow.x + VEL + yellow.width < BORDER.x + BORDER.width:
        yellow.x += VEL
...
def winning(text):
    draw_winner = WIN_FONT.render(text, True, WHITE)
    WIN.blit(draw_winner, (WIDTH // 2 - draw_winner.get_width() // 2, HEIGHT // 2 - draw_winner.get_height()))
    pygame.display.update()
